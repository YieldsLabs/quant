...
def create_candlestick(data: pd.DataFrame) -> go.Figure:
    ...
    long_win = (data["position.side"] == "long") & (
        data["position.exit_price"] > data["position.entry_price"]
    )
    long_loss = (data["position.side"] == "long") & (
        data["position.exit_price"] <= data["position.entry_price"]
    )
    ...
    traces.append(
        go.Scatter(
            ...
            mode="markers",
            marker={"symbol": "star", "size": 8, "color": entry_color_long},
            name="Long Win",
        )
    )
    ...
    traces.append(
        go.Scatter(
            ...
            mode="markers",
            marker={"symbol": "x", "size": 8, "color": entry_color_long},
            name="Long Loss",
        )
    )

    short_win = (data["position.side"] == "short") & (
        data["position.exit_price"] < data["position.entry_price"]
    )
    short_loss = (data["position.side"] == "short") & (
        data["position.exit_price"] >= data["position.entry_price"]
    )
    ...
    traces.append(
        go.Scatter(
            ...
            mode="markers",
            marker={"symbol": "star", "size": 8, "color": entry_color_short},
            name="Short Win",
        )
    )
    ...
    traces.append(
        go.Scatter(
            ...
            mode="markers",
            marker={"symbol": "x", "size": 8, "color": entry_color_short},
            name="Short Loss",
        )
    )
    ...
    for _, row in data.iterrows():
        if not pd.isna(row["position.entry_price"]):
            ...
            exit_timestamp = entry_timestamp + pd.Timedelta(
                row["position.trade_time"], unit="ms"
            )
            ...
            if "position.take_profit_price" in row and not pd.isna(
                row["position.take_profit_price"]
            ):
                shapes.append(
                    {
                        "type": "line",
            ...
            if "position.stop_loss_price" in row and not pd.isna(
                row["position.stop_loss_price"]
            ):
                shapes.append(
                    {
                        "type": "line",
...
def plot_candlestick(data: pd.DataFrame, charts_per_row: int = 2) -> go.Figure:
    ...
    fig.update_layout(
        ...
        plot_bgcolor="black",
        paper_bgcolor="black",
        margin={"autoexpand": False, "l": 40, "r": 15, "t": 30, "b": 30},
    )
...
def plot_equity_curve(df, symbol, timeframe, strategy):
    ...
    start_idx = 0
    current_color = (
        color_above_account if equity_curve[0] >= account_size else color_below_account
    )
    ...
    for idx, equity in enumerate(equity_curve[1:], 1):
        ...
        if color != current_color:
            add_equity_curve_segment(
                fig,
                trades[start_idx : idx + 1],
                equity_curve[start_idx : idx + 1],
                current_color,
            )
            start_idx = idx
            current_color = color
    ...
    add_equity_curve_segment(
        fig, trades[start_idx:], equity_curve[start_idx:], current_color
    )
    ...
    fig.update_layout(
        ...
        template="plotly_dark",
        showlegend=False,
        margin={"autoexpand": False, "l": 60, "r": 15, "t": 70, "b": 40},
