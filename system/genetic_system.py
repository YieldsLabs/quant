...
class GeneticSystem(AbstractSystem):
    ...
    async def _run_backtest(self):
        ...
        async for squad in self._generate_actors(self.population):
            ...
            if len(batch) == self.context.backtest_parallel:
                ...
                logger.info(
                    f"Estimated remaining time: {estimator.remaining_time():.2f} seconds"
                )
    ...
    async def _process_squad(self, squad: Squad):
        ...
        await self.dispatch(
            BacktestStarted(squad.symbol, squad.timeframe, squad.strategy)
        )
    ...
    async def _generate_actors(self, population, is_live=False):
        for individual in population:
            yield self.context.squad_factory.create_squad(
                individual.symbol, individual.timeframe, individual.strategy, is_live
            )
    ...
    async def _generate_population(self):
        ...
        filtered_symbols = [
            ...
            if symbol.name not in self.context.blacklist
        ]

        num_symbols_to_sample = min(self.context.sample_size, len(filtered_symbols))
        num_timeframes_to_sample = min(
            self.context.sample_size, len(self.context.timeframes)
        )

        sampled_symbols = np.random.choice(
            filtered_symbols, size=num_symbols_to_sample, replace=False
        )
        sampled_timeframes = np.random.choice(
            self.context.timeframes, size=num_timeframes_to_sample, replace=False
        )
