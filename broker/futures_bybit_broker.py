...
class FuturesBybitBroker(AbstractBroker):
    ...
    def set_settings(self, command: UpdateSettings):
        ...
        try:
            ...
            self.exchange.set_margin_mode(
                command.margin_mode.value, symbol, {"leverage": command.leverage}
            )
        except Exception:
            pass
    ...
    def open_position(self, command: OpenPosition):
        ...
        self._place_market_order(
            position.side, signal.symbol.name, position.size, None, None
        )
    ...
    def close_position(self, command: ClosePosition):
        ...
        self._create_order(
            "market",
            "sell" if open_position["position_side"] == PositionSide.LONG else "buy",
            symbol.name,
    ...
    def get_symbols(self, _query: GetSymbols):
        ...
        markets = [
            ...
            and not market_info["type"] == "future"
    ...
    def get_symbol(self, query: GetSymbol):
        return next(
            symbol for symbol in self.get_symbols() if symbol.name == query.name
        )

    def get_historical_data(self, symbol, timeframe, lookback, batch_size):
        start_time = (
            self.exchange.milliseconds()
            - lookback * self.exchange.parse_timeframe(timeframe) * 1000
        )
        ...
        while fetched_ohlcv < lookback:
            ...
            start_time = (
                current_ohlcv[-1][0] + self.exchange.parse_timeframe(timeframe) * 1000
            )
    ...
    def _place_market_order(
        self, side, symbol, position_size, stop_loss_price=None, take_profit_price=None
    ):
        order_params = self._create_order_params("market", side, symbol, position_size)
        order_params["extra_params"] = self._create_extra_params(
    ...
    def _place_limit_order(
        ...
        order_params = self._create_order_params(
            "limit", order_side, symbol, position_size
        )
        order_params.update(
            {
                "price": entry_price,
                "extra_params": self._create_extra_params(
                    stop_loss_price, take_profit_price
                ),
            }
        )
