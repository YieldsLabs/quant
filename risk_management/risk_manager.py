    ...
    async def _on_risk(self, event: RiskEvaluate):
        ...
        async with self.stop_loss_lock:
            if stop_loss_price:
                ...
                if self.risk_type == RiskType.BREAK_EVEN:
                    stop_loss_price = self.break_even_strategy.next(symbol, position_side, position_size, stop_loss_price, entry_price, ohlcv.low, ohlcv.high, risk_per_trade)

        if not self._should_exit(position_side, stop_loss_price, take_profit_price, ohlcv.low, ohlcv.high):
            return

        async with self.stop_loss_lock:
            self.break_even_strategy.reset(symbol, position_side)
